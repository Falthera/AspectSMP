...
public class ListenerManager implements Listener {
    ...
    public void onPlayerJoin(PlayerJoinEvent event) {
        plugin.getHeartManager().getOrCreateHeart(event.getPlayer().getUniqueId());
    }
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        ...
        if (event.getHand() == EquipmentSlot.OFF_HAND) {
            plugin.getAbilityManager().handleAbilityTrigger(event.getPlayer(), 
                plugin.getHeartManager().getHeart(event.getPlayer().getUniqueId()).orElse(null), 
                event.getHand());
        }
